"""RandomForest on top-10 behavioral + 17 metadata features (the new dataset).

Same pipeline as ``randomforest2.py`` -- time-ordered grouped
train/val/test with a pinned test market, 5-fold TimeSeriesSplit CV on
train, fixed threshold 0.5, PR/ROC scatter plots, sorted-prediction log,
sklearn RandomForestClassifier wrapped in a Pipeline + SimpleImputer (the
metadata block carries NaNs, which RF cannot ingest natively) -- but it
trains on ``cache/training_data_with_metadata.parquet`` and the feature
set is the top-10 behavioral features used by rf2 PLUS all 17 metadata
features. This isolates the lift the metadata block adds on top of the
established top-10 behavioral model.

Outputs:
  cache/models/rf_insider_meta_v3_<ts>.joblib              (Pipeline)
  cache/models/rf_insider_meta_v3_<ts>.meta.json
  cache/models/rf_insider_meta_v3_latest.joblib            (copies)
  cache/models/rf_insider_meta_v3_latest.meta.json
  randomforest3_<ts>.log                                   (full stdout+stderr)
  randomforest3_<ts>_curves.png
"""
...
TOP_K = 10
MODEL_TAG = __import__("os").environ.get("MODEL_TAG_PREFIX", "") + "rf_insider_meta_v3"
...
def get_feature_columns() -> list[str]:
    ...
    print(f"[rf3] using {len(behavioral)} behavioral + {len(metadata)} metadata "
          f"= {len(combined)} features")
    print(f"[rf3] behavioral (top-{TOP_K} from {FEATURE_SOURCE_META.name}):")
    for i, name in enumerate(behavioral, 1):
        print(f"  {i:2d}. {name}")
    print(f"[rf3] metadata:")
...
def cv_on_train(
    ...
    print(f"[rf3] running {N_CV_FOLDS}-fold expanding-window CV on "
    ...
    print(f"[rf3] CV ROC-AUC = {summary['roc_auc_mean']:.4f} "
          f"± {summary['roc_auc_std']:.4f}  "
          f"(over {summary['n_folds_with_metric']} usable folds)")
    print(f"[rf3] CV PR-AUC  = {summary['pr_auc_mean']:.4f} "
...
def eval_split(
    ...
    out["f1"] = cls["f1"]
    print(f"[rf3] {label:5s}  n={out['n']:4d} pos={out['n_positives']:3d}  "
...
def main() -> None:
    ...
    try:
        ...
        log_file.close()
        print(f"[rf3] log written to {log_path}")


def _main(ts: str, log_path: Path) -> None:
    print(f"[rf3] run timestamp = {ts}")
    print(f"[rf3] logging to {log_path}")
    print(f"[rf3] loading {TRAIN_PARQUET}")
    if not TRAIN_PARQUET.exists():
        raise SystemExit(
            f"missing {TRAIN_PARQUET}; run build_dataset_with_metadata.py first"
        )
    df = pd.read_parquet(TRAIN_PARQUET)
    print(f"[rf3] full shape={df.shape}  "
    ...
    print(f"[rf3] markets total: {len(market_order)}  "
          f"(train={len(train_mks)}, val={len(val_mks)}, test={len(test_mks)})")
    print("[rf3] market order (oldest -> newest), split assignment:")
    ...
    print(f"[rf3] row distribution (total n={n_total}, pos={pos_total}, "
    ...
    print(f"[rf3] final positive class weight = {POSITIVE_CLASS_WEIGHT:.3f}  "
    ...
    print(f"[rf3] fit in {time.time() - t0:.2f}s  "
    ...
    print(f"[rf3] prob range  train={_rng(proba_train)}  "
          f"val={_rng(proba_val)}  test={_rng(proba_test)}")
    print()

    print("[rf3] metrics per split (overfit check: train >> val => overfit):")
    ...
    print(f"[rf3] PR-AUC gap train-val = {train_val_gap:+.4f}  "
          f"(big positive = overfit train)")
    print(f"[rf3] PR-AUC gap val-test  = {val_test_gap:+.4f}  "
    ...
    print()
    print(f"[rf3] classification metrics @ fixed thr={THRESHOLD}:")
    ...
    print(f"[rf3] curves saved to {curves_path}")
    print()

    print(f"[rf3] TEST predictions, one row per wallet, sorted by prob desc.")
    print(f"[rf3]  rank  prob    label  cum_tp  cum_fp  prec   recall  wallet")
    ...
    for rank, k in enumerate(order, start=1):
        ...
        print(f"[rf3]  {rank:>4}    {proba_test[k]:.4f}    {lab}     "
              f"{cum_tp:>4}    {cum_fp:>4}  {prec:.3f}   {rec:.3f}  "
              f"{wallet_str}")
    print()

    print(f"[rf3] TEST confusion matrix @ thr={THRESHOLD}")
    ...
    print("[rf3] feature importances (mean decrease in impurity):")
    ...
    print()
    print(f"[rf3] saved {model_path}")
    print(f"[rf3] saved {meta_path}")
    print(f"[rf3] copied -> {latest_model}")
    print(f"[rf3] copied -> {latest_meta}")
    print()
    print("[rf3] comparison vs other models (test-set ROC-AUC / PR-AUC):")
    comparisons = [
    ...
    for tag, path in comparisons:
        try:
            ...
            pass
    print(f"  {'rf  10-feat + meta v3 (this run)':<35}  "
