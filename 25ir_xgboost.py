"""XGBoost (top-20 behavioral + top-10 metadata), Iran-reduced dataset.

Experiment variant of ``xgboost7.py``. Tests whether the canonical Iran market
is over-represented and corrupting the model by DOWN-SAMPLING it: the market
``us-strikes-iran-by-march-1-2026-492`` (245 rows / 35 ins / 210 non) keeps
only 10 insiders + 60 non-insiders (25 insiders hidden, ratio preserved). The
second big Iran market (feb-28) and the 3 tiny Iran markets are untouched. See
``0ir_xgboost.py`` for the all-big-Iran-removed variant.

Unlike xgboost7 (pins Iran as the test market), this script uses a single
seeded stratified-by-group split (GroupShuffleSplit ~15%) because Iran can no
longer be the pinned test market. CROSS-VALIDATION PR-AUC (StratifiedGroupKFold
over the training set) is the primary metric; held-out test PR-AUC is the
secondary, higher-variance number. Same Optuna tuning + OOF Platt calibration
as xgboost7.

Outputs:
  cache/models/xgb_insider_25ir_<ts>.json           (XGBoost native fmt)
  cache/models/xgb_insider_25ir_<ts>.calibrator.joblib
  cache/models/xgb_insider_25ir_<ts>.meta.json
  25ir_xgboost_<ts>_curves.png
  25ir_xgboost_<ts>.log
"""
...
TOP_K = 20
MODEL_TAG = __import__("os").environ.get("MODEL_TAG_PREFIX", "") + "xgb_insider_25ir"
LOG_PREFIX = "25ir_xgboost"
TAG = "xgb25ir"
