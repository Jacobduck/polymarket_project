"""XGBoost on top-10 behavioral + 17 metadata features (the new dataset).

Same pipeline as ``xgboost3.py`` -- Optuna-tuned XGBClassifier, OOF Platt
calibration, StratifiedGroupKFold CV, pinned iran-mar-1 test set, PR/ROC
scatter plots, sorted-prediction log -- but it trains on
``cache/training_data_with_metadata.parquet`` and the feature set is the
top-10 behavioral features used by rf2/rf3 PLUS all 17 metadata features.
The Iran test pin matches randomforest3.py so xgb-vs-rf on the metadata
dataset is apples-to-apples. XGBoost ingests NaN natively, so (unlike the
RF pipeline) no imputation is applied to the NaN-heavy metadata block.

Outputs:
  cache/models/xgb_insider_meta_v5_<ts>.json               (XGBoost native fmt)
  cache/models/xgb_insider_meta_v5_<ts>.calibrator.joblib
  cache/models/xgb_insider_meta_v5_<ts>.meta.json
  cache/models/xgb_insider_meta_v5_latest.json             (copies)
  cache/models/xgb_insider_meta_v5_latest.calibrator.joblib
  cache/models/xgb_insider_meta_v5_latest.meta.json
  xgboost5_<ts>.log                                        (full stdout+stderr)
  xgboost5_<ts>_curves.png
"""
...
TOP_K = 10
MODEL_TAG = __import__("os").environ.get("MODEL_TAG_PREFIX", "") + "xgb_insider_meta_v5"
...
def get_feature_columns() -> list[str]:
    ...
    Behavioral block reuses the same top-K (=10) ranking rf3 used (first K
    entries of cb_insider_30feat_latest.meta.json, sorted by xgb gain);
    the metadata block is appended in METADATA_FEATURE_COLUMNS order.
    """
    ...
    print(f"[xgb5] using {len(behavioral)} behavioral + {len(metadata)} metadata "
          f"= {len(combined)} features")
    print(f"[xgb5] behavioral (top-{TOP_K} from {FEATURE_SOURCE_META.name}):")
    for i, name in enumerate(behavioral, 1):
        print(f"  {i:2d}. {name}")
    print(f"[xgb5] metadata:")
...
def split_markets_stratified_random(
    ...
    if PIN_TEST_MARKETS:
        ...
        print(f"[xgb5] PIN_TEST_MARKETS active -> "
...
def eval_split(
    ...
    out["f1"] = cls["f1"]
    print(f"[xgb5] {label:5s}  n={out['n']:4d} pos={out['n_positives']:3d}  "
...
def main() -> None:
    ...
    try:
        ...
        log_file.close()
        print(f"[xgb5] log written to {log_path}")


def _main(ts: str, log_path: Path) -> None:
    print(f"[xgb5] run timestamp = {ts}")
    print(f"[xgb5] logging to {log_path}")
    print(f"[xgb5] loading {TRAIN_PARQUET}")
    if not TRAIN_PARQUET.exists():
        raise SystemExit(
            f"missing {TRAIN_PARQUET}; run build_dataset_with_metadata.py first"
        )
    df = pd.read_parquet(TRAIN_PARQUET)
    print(f"[xgb5] full shape={df.shape}  "
    ...
    print(f"[xgb5] markets total: {len(stats)}  "
          f"(train={len(train_mks)}, test={len(test_mks)})")
    print("[xgb5] market assignment (sorted by row count desc):")
    ...
    print(f"[xgb5] row distribution (total n={n_total}, pos={pos_total}, "
    ...
    print(f"[xgb5] starting Optuna tuning: {N_OPTUNA_TRIALS} trials, "
    ...
    def _log_cb(study_: optuna.Study, trial: optuna.trial.FrozenTrial) -> None:
        ...
        print(f"[xgb5]   trial {trial.number:>3}  state={state:<8}  "
    ...
    print(f"[xgb5] tuning done in {tuning_secs:.1f}s  "
          f"({n_completed} completed, {n_pruned} pruned)")
    print(f"[xgb5] best CV PR-AUC = {best_cv_pr_auc:.4f}")
    print("[xgb5] best params:")
    ...
    print()
    print(f"[xgb5] re-running CV with best params to collect OOF preds + per-fold scores")
    splitter = StratifiedGroupKFold(
        n_splits=N_CV_FOLDS, shuffle=True, random_state=RANDOM_STATE,
    )
    ...
    print(f"[xgb5] CV PR-AUC = {cv_pr_auc_mean:.4f} ± {cv_pr_auc_std:.4f}")
    ...
    print(f"[xgb5] OOF Platt calibrator: "
          f"sigmoid({cal_coef:.3f} * raw_prob {cal_intercept:+.3f})")
    print()

    print(f"[xgb5] fitting final model on full train ({len(X_train)} rows)")
    final_model = build_xgb(best_params)
    t1 = time.time()
    final_model.fit(X_train, y_train, verbose=False)
    print(f"[xgb5] final fit in {time.time() - t1:.2f}s")
    print()
    ...
    print(f"[xgb5] raw  prob range  train={_rng(proba_train_raw)}  "
          f"test={_rng(proba_test_raw)}")
    print(f"[xgb5] cal. prob range  train={_rng(proba_train)}  "
    ...
    print(f"[xgb5] sanity: TEST PR-AUC raw={raw_test_pr:.4f}  "
          f"(should equal calibrated PR-AUC; AUC is invariant to Platt)")
    print()

    print("[xgb5] metrics per split (overfit check: train >> test => overfit):")
    ...
    print(f"[xgb5] PR-AUC gap train-test = {train_test_gap:+.4f}  "
          f"(big positive = overfit train)")
    print(f"[xgb5] PR-AUC gap CV-test    = {cv_test_gap:+.4f}  "
    ...
    print()
    print(f"[xgb5] classification metrics @ fixed thr={THRESHOLD}:")
    ...
    print(f"[xgb5] curves saved to {curves_path}")
    print()

    print("[xgb5] TEST predictions, one row per wallet, sorted by prob desc.")
    print("[xgb5]  rank  prob    label  cum_tp  cum_fp  prec   recall  market  wallet")
    ...
    for rank, k in enumerate(order, start=1):
        ...
        print(f"[xgb5]  {rank:>4}    {proba_test[k]:.4f}    {lab}     "
              f"{cum_tp:>4}    {cum_fp:>4}  {prec:.3f}   {rec:.3f}  "
              f"{mkt_str:<30}  {wallet_str}")
    print()

    print(f"[xgb5] TEST confusion matrix @ thr={THRESHOLD}")
    ...
    print("[xgb5] feature importances (gain):")
    ...
    print()
    print(f"[xgb5] saved {model_path}")
    print(f"[xgb5] saved {calibrator_path}")
    print(f"[xgb5] saved {meta_path}")
    print(f"[xgb5] copied -> {latest_model}")
    print(f"[xgb5] copied -> {latest_calibrator}")
    print(f"[xgb5] copied -> {latest_meta}")
    print()
    print("[xgb5] comparison vs other models on each model's own test set "
    ...
    for tag, path in comparisons:
        try:
            ...
            pass
    print(f"  {'xgb 10-feat + meta v5 (this run)':<37}  "
