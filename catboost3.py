"""Train CatBoost on the top-10 features (same pipeline as catboost2.py).

Identical to ``catboost2.py`` except features are the top 10 (by gain) from
``cb_insider_30feat_latest.meta.json`` (which is the top-30 list sorted by
xgb gain). Everything else -- time-ordered grouped train/val/test split,
TimeSeriesSplit CV on train, Platt calibration on val, fixed threshold,
PR/ROC scatter plots, sorted-prediction log -- is unchanged.

Outputs:
  cache/models/cb_insider_10feat_v2_<ts>.joblib              (CatBoost model)
  cache/models/cb_insider_10feat_v2_<ts>.calibrator.joblib   (sklearn LR)
  cache/models/cb_insider_10feat_v2_<ts>.meta.json
  cache/models/cb_insider_10feat_v2_latest.joblib            (copies)
  cache/models/cb_insider_10feat_v2_latest.calibrator.joblib
  cache/models/cb_insider_10feat_v2_latest.meta.json
  catboost3_<ts>.log                                         (full stdout+stderr)
"""
...
def get_top_features() -> list[str]:
    ...
    top = feats[:TOP_K]
    print(f"[cb3] using top-{TOP_K} features from {FEATURE_SOURCE_META.name}:")
...
def cv_on_train(
    ...
    print(f"[cb3] running {N_CV_FOLDS}-fold expanding-window CV on "
    ...
    print(f"[cb3] CV ROC-AUC = {summary['roc_auc_mean']:.4f} "
          f"± {summary['roc_auc_std']:.4f}  "
          f"(over {summary['n_folds_with_metric']} usable folds)")
    print(f"[cb3] CV PR-AUC  = {summary['pr_auc_mean']:.4f} "
...
def eval_split(
    ...
    out["f1"] = cls["f1"]
    print(f"[cb3] {label:5s}  n={out['n']:4d} pos={out['n_positives']:3d}  "
...
def main() -> None:
    ...
    try:
        ...
        log_file.close()
        print(f"[cb3] log written to {log_path}")


def _main(ts: str, log_path: Path) -> None:
    print(f"[cb3] run timestamp = {ts}")
    print(f"[cb3] logging to {log_path}")
    print(f"[cb3] loading {TRAIN_PARQUET}")
    df = pd.read_parquet(TRAIN_PARQUET)
    print(f"[cb3] full shape={df.shape}  "
    ...
    print(f"[cb3] markets total: {len(market_order)}  "
          f"(train={len(train_mks)}, val={len(val_mks)}, test={len(test_mks)})")
    print("[cb3] market order (oldest -> newest), split assignment:")
    ...
    print(f"[cb3] row distribution (total n={n_total}, pos={pos_total}, "
    ...
    print(f"[cb3] final train scale_pos_weight = {scale_pos_weight:.3f}  "
    ...
    best_iter = int(model.get_best_iteration() or 0)
    print(f"[cb3] fit in {time.time() - t0:.2f}s  best_iter={best_iter} / "
          f"{MAX_ITERATIONS}")
    print()
    ...
    print(f"[cb3] calibration: sigmoid({cal_coef:.3f} * raw_prob "
    ...
    print(f"[cb3] raw  prob range  train={_rng(proba_train_raw)}  "
          f"val={_rng(proba_val_raw)}  test={_rng(proba_test_raw)}")
    print(f"[cb3] cal. prob range  train={_rng(proba_train)}  "
          f"val={_rng(proba_val)}  test={_rng(proba_test)}")
    print()

    print("[cb3] metrics per split (overfit check: train >> val => overfit):")
    ...
    print(f"[cb3] PR-AUC gap train-val = {train_val_gap:+.4f}  "
          f"(big positive = overfit train)")
    print(f"[cb3] PR-AUC gap val-test  = {val_test_gap:+.4f}  "
    ...
    print()
    print(f"[cb3] classification metrics @ fixed thr={THRESHOLD}:")
    ...
    print(f"[cb3] curves saved to {curves_path}")
    print()

    print(f"[cb3] TEST predictions, one row per wallet, sorted by cal_prob desc.")
    print(f"[cb3]  rank  cal_prob  raw_prob  label  cum_tp  cum_fp  prec   recall  wallet")
    ...
    for rank, k in enumerate(order, start=1):
        ...
        print(f"[cb3]  {rank:>4}    {proba_test[k]:.4f}    "
              f"{proba_test_raw[k]:.4f}    {lab}     {cum_tp:>4}    "
              f"{cum_fp:>4}  {prec:.3f}   {rec:.3f}  {wallet_str}")
    print()

    print(f"[cb3] TEST confusion matrix @ thr={THRESHOLD}")
    ...
    print("[cb3] feature importances:")
    for name, imp in importances:
        print(f"  {imp:7.4f}  {name}")

    model_path = MODEL_DIR / f"cb_insider_10feat_v2_{ts}.joblib"
    meta_path = MODEL_DIR / f"cb_insider_10feat_v2_{ts}.meta.json"
    calibrator_path = MODEL_DIR / f"cb_insider_10feat_v2_{ts}.calibrator.joblib"
    ...
    latest_model = MODEL_DIR / "cb_insider_10feat_v2_latest.joblib"
    latest_meta = MODEL_DIR / "cb_insider_10feat_v2_latest.meta.json"
    latest_calibrator = MODEL_DIR / "cb_insider_10feat_v2_latest.calibrator.joblib"
    shutil.copy(model_path, latest_model)
    shutil.copy(meta_path, latest_meta)
    shutil.copy(calibrator_path, latest_calibrator)

    print()
    print(f"[cb3] saved {model_path}")
    print(f"[cb3] saved {calibrator_path}")
    print(f"[cb3] saved {meta_path}")
    print(f"[cb3] copied -> {latest_model}")
    print(f"[cb3] copied -> {latest_calibrator}")
    print(f"[cb3] copied -> {latest_meta}")
    print()
    print("[cb3] comparison vs catboost2 (30-feat, same pipeline):")
    try:
        with open(MODEL_DIR / "cb_insider_30feat_v2_latest.meta.json") as f:
            prior = json.load(f).get("metrics", {})
        print(f"  30-feat v2:  ROC-AUC={prior.get('roc_auc', 0):.4f}  "
              f"PR-AUC={prior.get('pr_auc', 0):.4f}")
    except FileNotFoundError:
        pass
    print(f"  10-feat v2:  ROC-AUC={test_metrics['roc_auc']:.4f}  "
