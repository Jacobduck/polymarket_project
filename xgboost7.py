"""XGBoost on top-20 behavioral + top-10 metadata features (the new dataset).

Same pipeline as ``xgboost5.py`` -- Optuna-tuned XGBClassifier, OOF Platt
calibration, StratifiedGroupKFold CV, pinned iran-mar-1 test set, PR/ROC
scatter plots, sorted-prediction log -- but the feature set is the top-20
behavioral features PLUS the top-10 metadata features (ranked by xgb5's
gain importance). This drops the 7 weakest metadata features (mostly the
NaN-heavy funder-graph block) while widening the behavioral block from 10
to 20. The Iran test pin matches randomforest3.py / xgboost5.py so the
comparison stays apples-to-apples. XGBoost ingests NaN natively, so no
imputation is applied to the NaN-heavy metadata block.

Outputs:
  cache/models/xgb_insider_meta_v7_<ts>.json               (XGBoost native fmt)
  cache/models/xgb_insider_meta_v7_<ts>.calibrator.joblib
  cache/models/xgb_insider_meta_v7_<ts>.meta.json
  cache/models/xgb_insider_meta_v7_latest.json             (copies)
  cache/models/xgb_insider_meta_v7_latest.calibrator.joblib
  cache/models/xgb_insider_meta_v7_latest.meta.json
  xgboost7_<ts>.log                                        (full stdout+stderr)
  xgboost7_<ts>_curves.png
"""
...
TOP_K = 20
MODEL_TAG = __import__("os").environ.get("MODEL_TAG_PREFIX", "") + "xgb_insider_meta_v7"

# Top 10 of the 17 metadata features, ranked by xgb5's gain importance
# (xgb5 = top-10 behavioral + 17 metadata). Drops the 7 weakest, which are
# dominated by the NaN-heavy funder-graph block.
...
# Pin the same test market as randomforest3.py for a direct xgb-vs-rf
# comparison on the metadata dataset.
...
def get_feature_columns() -> list[str]:
    """Top-K (=20) behavioral features + top-10 metadata features.

    Behavioral block is the first K entries of
    cb_insider_30feat_latest.meta.json (sorted by xgb gain); the metadata
    block is TOP_METADATA_FEATURES (top 10 of 17, ranked by xgb5 gain).
    """
    ...
    print(f"[xgb7] using {len(behavioral)} behavioral + {len(metadata)} metadata "
          f"= {len(combined)} features")
    print(f"[xgb7] behavioral (top-{TOP_K} from {FEATURE_SOURCE_META.name}):")
    for i, name in enumerate(behavioral, 1):
        print(f"  {i:2d}. {name}")
    print(f"[xgb7] metadata (top 10 of 17, ranked by xgb5 gain):")
...
def split_markets_stratified_random(
    ...
    test set. Otherwise: markets are partitioned by row count into "large"
    ...
    if PIN_TEST_MARKETS:
        ...
        print(f"[xgb7] PIN_TEST_MARKETS active -> "
...
def eval_split(
    ...
    out["f1"] = cls["f1"]
    print(f"[xgb7] {label:5s}  n={out['n']:4d} pos={out['n_positives']:3d}  "
...
def main() -> None:
    ...
    try:
        ...
        log_file.close()
        print(f"[xgb7] log written to {log_path}")


def _main(ts: str, log_path: Path) -> None:
    print(f"[xgb7] run timestamp = {ts}")
    print(f"[xgb7] logging to {log_path}")
    print(f"[xgb7] loading {TRAIN_PARQUET}")
    if not TRAIN_PARQUET.exists():
        raise SystemExit(
            f"missing {TRAIN_PARQUET}; run build_dataset_with_metadata.py first"
        )
    df = pd.read_parquet(TRAIN_PARQUET)
    print(f"[xgb7] full shape={df.shape}  "
    ...
    print(f"[xgb7] markets total: {len(stats)}  "
          f"(train={len(train_mks)}, test={len(test_mks)})")
    print("[xgb7] market assignment (sorted by row count desc):")
    ...
    print(f"[xgb7] row distribution (total n={n_total}, pos={pos_total}, "
    ...
    print(f"[xgb7] starting Optuna tuning: {N_OPTUNA_TRIALS} trials, "
    ...
    def _log_cb(study_: optuna.Study, trial: optuna.trial.FrozenTrial) -> None:
        ...
        print(f"[xgb7]   trial {trial.number:>3}  state={state:<8}  "
    ...
    print(f"[xgb7] tuning done in {tuning_secs:.1f}s  "
          f"({n_completed} completed, {n_pruned} pruned)")
    print(f"[xgb7] best CV PR-AUC = {best_cv_pr_auc:.4f}")
    print("[xgb7] best params:")
    ...
    print()
    print(f"[xgb7] re-running CV with best params to collect OOF preds + per-fold scores")
    splitter = StratifiedGroupKFold(
        n_splits=N_CV_FOLDS, shuffle=True, random_state=RANDOM_STATE,
    )
    ...
    print(f"[xgb7] CV PR-AUC = {cv_pr_auc_mean:.4f} ± {cv_pr_auc_std:.4f}")
    ...
    # will not move; only thresholded metrics (P / R / F1) shift.
    ...
    print(f"[xgb7] OOF Platt calibrator: "
          f"sigmoid({cal_coef:.3f} * raw_prob {cal_intercept:+.3f})")
    print()

    print(f"[xgb7] fitting final model on full train ({len(X_train)} rows)")
    final_model = build_xgb(best_params)
    t1 = time.time()
    final_model.fit(X_train, y_train, verbose=False)
    print(f"[xgb7] final fit in {time.time() - t1:.2f}s")
    print()
    ...
    print(f"[xgb7] raw  prob range  train={_rng(proba_train_raw)}  "
          f"test={_rng(proba_test_raw)}")
    print(f"[xgb7] cal. prob range  train={_rng(proba_train)}  "
    ...
    print(f"[xgb7] sanity: TEST PR-AUC raw={raw_test_pr:.4f}  "
          f"(should equal calibrated PR-AUC; AUC is invariant to Platt)")
    print()

    print("[xgb7] metrics per split (overfit check: train >> test => overfit):")
    ...
    print(f"[xgb7] PR-AUC gap train-test = {train_test_gap:+.4f}  "
          f"(big positive = overfit train)")
    print(f"[xgb7] PR-AUC gap CV-test    = {cv_test_gap:+.4f}  "
          f"(big positive = test market harder than CV folds)")
    ...
    print()
    print(f"[xgb7] classification metrics @ fixed thr={THRESHOLD}:")
    ...
    print(f"[xgb7] curves saved to {curves_path}")
    print()

    print("[xgb7] TEST predictions, one row per wallet, sorted by prob desc.")
    print("[xgb7]  rank  prob    label  cum_tp  cum_fp  prec   recall  market  wallet")
    ...
    for rank, k in enumerate(order, start=1):
        ...
        print(f"[xgb7]  {rank:>4}    {proba_test[k]:.4f}    {lab}     "
              f"{cum_tp:>4}    {cum_fp:>4}  {prec:.3f}   {rec:.3f}  "
              f"{mkt_str:<30}  {wallet_str}")
    print()

    print(f"[xgb7] TEST confusion matrix @ thr={THRESHOLD}")
    ...
    print("[xgb7] feature importances (gain):")
    ...
    print()
    print(f"[xgb7] saved {model_path}")
    print(f"[xgb7] saved {calibrator_path}")
    print(f"[xgb7] saved {meta_path}")
    print(f"[xgb7] copied -> {latest_model}")
    print(f"[xgb7] copied -> {latest_calibrator}")
    print(f"[xgb7] copied -> {latest_meta}")
    print()
    print("[xgb7] comparison vs other models on each model's own test set "
          "(rf3 / this run both pin iran-mar-1 -> directly comparable):")
    comparisons = [
        ("xgb 10-feat + meta v5 (combined, iran)",  MODEL_DIR / "xgb_insider_meta_v5_latest.meta.json"),
        ("xgb metadata-only v6 (iran)",             MODEL_DIR / "xgb_insider_meta10_v6_latest.meta.json"),
        ("rf  10-feat + meta v3 (iran test)",       MODEL_DIR / "rf_insider_meta_v3_latest.meta.json"),
        ("xgb 20-feat v2 (behavioral, iran)",       MODEL_DIR / "xgb_insider_20feat_v2_latest.meta.json"),
        ("cb  10-feat v2 (behavioral)",             MODEL_DIR / "cb_insider_10feat_v2_latest.meta.json"),
        ("cb  30-feat v2 (behavioral)",             MODEL_DIR / "cb_insider_30feat_v2_latest.meta.json"),
    ]
    for tag, path in comparisons:
        try:
            with open(path) as f:
                m = json.load(f).get("metrics", {})
            print(f"  {tag:<42}  ROC-AUC={m.get('roc_auc', 0):.4f}  "
                  f"PR-AUC={m.get('pr_auc', 0):.4f}")
        except FileNotFoundError:
            pass
    print(f"  {'xgb 20-feat + meta10 v7 (this run)':<42}  "
