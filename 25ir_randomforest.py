"""RandomForest (top-10 behavioral + top-10 metadata), Iran-reduced dataset.

Experiment variant of ``randomforest5.py`` that tests the hypothesis that the
canonical Iran market is over-represented and corrupting the model. Here the
canonical Iran market ``us-strikes-iran-by-march-1-2026-492`` (245 rows / 35
insiders / 210 non-insiders) is DOWN-SAMPLED: 25 of its 35 insider rows are
hidden, and non-insiders are dropped alongside to preserve the market's
original ~14.3% insider ratio -> 10 insiders + 60 non-insiders = 70 rows kept.
The second big Iran market (feb-28) and the 3 tiny Iran markets are left
untouched. See ``0ir_randomforest.py`` for the all-Iran-removed variant.

Unlike randomforest5 (loops over pinned test markets, one of which is Iran),
this script uses a single seeded stratified-by-group train/test split
(GroupShuffleSplit ~15%), because Iran can no longer serve as the pinned test
market. CROSS-VALIDATION PR-AUC (GroupKFold over the training set) is the
primary metric; the held-out test PR-AUC is reported as a secondary,
higher-variance number.
...
Outputs:
  cache/models/rf_insider_25ir_<ts>.joblib        (Pipeline)
  cache/models/rf_insider_25ir_<ts>.meta.json
  25ir_randomforest_<ts>_curves.png
  25ir_randomforest_<ts>.log
"""
...
TOP_K = 10
MODEL_TAG = __import__("os").environ.get("MODEL_TAG_PREFIX", "") + "rf_insider_25ir"
LOG_PREFIX = "25ir_randomforest"
TAG = "rf25ir"
...
HIDE_INSIDERS = 25  # hide 25 of the 35 mar-1 insiders -> keep 10
...
def filter_iran(df: pd.DataFrame) -> pd.DataFrame:
    """Down-sample the canonical mar-1 Iran market, preserving its ratio."""
    sub = df[df[GROUP_COL] == MAR1_MARKET]
    if len(sub) == 0:
        raise SystemExit(f"canonical Iran market not in data: {MAR1_MARKET}")
    ins = sub[sub[LABEL_COL] == 1]
    non = sub[sub[LABEL_COL] == 0]
    orig_pos, orig_neg = len(ins), len(non)
    keep_ins = orig_pos - HIDE_INSIDERS
    if keep_ins < 1:
        raise SystemExit(
            f"HIDE_INSIDERS={HIDE_INSIDERS} leaves <1 insider (have {orig_pos})"
        )
    # Preserve the market's original insider ratio: keep_non/keep_ins ~ orig.
    keep_non = int(round(keep_ins * orig_neg / max(orig_pos, 1)))
    keep_non = min(keep_non, orig_neg)
    keep_ins_idx = ins.sample(n=keep_ins, random_state=RANDOM_STATE).index
    keep_non_idx = non.sample(n=keep_non, random_state=RANDOM_STATE).index
    drop_idx = set(sub.index) - set(keep_ins_idx) - set(keep_non_idx)
    out = df.drop(index=list(drop_idx)).copy()
    print(f"[{TAG}] Iran filter = 25ir (down-sample canonical mar-1)")
    print(f"[{TAG}]   mar-1 before: {len(sub)} rows "
          f"({orig_pos} ins / {orig_neg} non, {100*orig_pos/len(sub):.1f}% ins)")
    print(f"[{TAG}]   mar-1 after : {keep_ins + keep_non} rows "
          f"({keep_ins} ins / {keep_non} non, "
          f"{100*keep_ins/max(keep_ins+keep_non,1):.1f}% ins)  "
          f"-> hid {HIDE_INSIDERS} ins + {orig_neg-keep_non} non")
...
def plot_curves(
    ...
    ax_roc.set_title("ROC curves (25ir: Iran down-sampled)")
...
def _main(ts: str, log_path: Path) -> None:
    ...
    meta = {
        ...
        "experiment": "25ir -- canonical mar-1 Iran market down-sampled",
        "iran_filter": {
            "mode": "25ir",
            "market": MAR1_MARKET,
            "hidden_insiders": HIDE_INSIDERS,
            "note": "kept 10 insiders + 60 non-insiders (ratio preserved); "
                    "feb-28 and tiny Iran markets untouched",
        },
        "split_strategy": (
