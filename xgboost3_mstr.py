"""xgb3 top-20 with Platt cal, tested on `microstrategy-announces-1000-btc-purchase-december-2-8`.

Identical pipeline to xgboost3.py (top 20 features, Optuna, OOF Platt cal,
StratifiedGroupKFold) but with the test market pinned to the MicroStrategy
Dec 2-8 BTC-purchase market instead of iran-mar-1.

Outputs:
  cache/models/xgb_insider_20feat_v2_mstr_<ts>.json
  cache/models/xgb_insider_20feat_v2_mstr_<ts>.calibrator.joblib
  cache/models/xgb_insider_20feat_v2_mstr_<ts>.meta.json
  cache/models/xgb_insider_20feat_v2_mstr_latest.json (copies)
  cache/models/xgb_insider_20feat_v2_mstr_latest.calibrator.joblib
  cache/models/xgb_insider_20feat_v2_mstr_latest.meta.json
  xgboost3_mstr_<ts>.log
  xgboost3_mstr_<ts>_curves.png
"""
...
TOP_K = 20
MODEL_TAG = f"xgb_insider_{TOP_K}feat_v2_mstr"
...
def get_top_features() -> list[str]:
    ...
    top = feats[:TOP_K]
    print(f"[xgb3-mstr] using top-{TOP_K} features from {FEATURE_SOURCE_META.name}:")
...
def split_markets_stratified_random(
    ...
    if PIN_TEST_MARKETS:
        ...
        print(f"[xgb3-mstr] PIN_TEST_MARKETS active -> "
...
def eval_split(
    ...
    out["f1"] = cls["f1"]
    print(f"[xgb3-mstr] {label:5s}  n={out['n']:4d} pos={out['n_positives']:3d}  "
...
def main() -> None:
    ...
    try:
        ...
        log_file.close()
        print(f"[xgb3-mstr] log written to {log_path}")


def _main(ts: str, log_path: Path) -> None:
    print(f"[xgb3-mstr] run timestamp = {ts}")
    print(f"[xgb3-mstr] logging to {log_path}")
    print(f"[xgb3-mstr] loading {TRAIN_PARQUET}")
    df = pd.read_parquet(TRAIN_PARQUET)
    print(f"[xgb3-mstr] full shape={df.shape}  "
    ...
    print(f"[xgb3-mstr] row distribution (total n={n_total}, pos={pos_total}, "
    ...
    print(f"[xgb3-mstr] starting Optuna tuning: {N_OPTUNA_TRIALS} trials, "
    ...
    def _log_cb(study_: optuna.Study, trial: optuna.trial.FrozenTrial) -> None:
        ...
        print(f"[xgb3-mstr]   trial {trial.number:>3}  state={state:<8}  "
    ...
    print(f"[xgb3-mstr] tuning done in {tuning_secs:.1f}s  "
          f"({n_completed} completed, {n_pruned} pruned)")
    print(f"[xgb3-mstr] best CV PR-AUC = {best_cv_pr_auc:.4f}")
    print("[xgb3-mstr] best params:")
    ...
    print()
    print(f"[xgb3-mstr] re-running CV with best params to collect OOF preds + per-fold scores")
    splitter = StratifiedGroupKFold(
        n_splits=N_CV_FOLDS, shuffle=True, random_state=RANDOM_STATE,
    )
    ...
    print(f"[xgb3-mstr] CV PR-AUC = {cv_pr_auc_mean:.4f} ± {cv_pr_auc_std:.4f}")
    ...
    print(f"[xgb3-mstr] OOF Platt calibrator: "
          f"sigmoid({cal_coef:.3f} * raw_prob {cal_intercept:+.3f})")
    print()

    print(f"[xgb3-mstr] fitting final model on full train ({len(X_train)} rows)")
    final_model = build_xgb(best_params)
    t1 = time.time()
    final_model.fit(X_train, y_train, verbose=False)
    print(f"[xgb3-mstr] final fit in {time.time() - t1:.2f}s")
    print()
    ...
    print(f"[xgb3-mstr] raw  prob range  train={_rng(proba_train_raw)}  "
          f"test={_rng(proba_test_raw)}")
    print(f"[xgb3-mstr] cal. prob range  train={_rng(proba_train)}  "
    ...
    raw_test_roc = float(roc_auc_score(y_test.values, proba_test_raw)) \
        if len(np.unique(y_test.values)) >= 2 else float("nan")
    print(f"[xgb3-mstr] sanity: TEST PR-AUC raw={raw_test_pr:.4f}  "
          f"(should equal calibrated PR-AUC; AUC is invariant to Platt)")
    print()

    print("[xgb3-mstr] metrics per split (overfit check: train >> test => overfit):")
    ...
    print(f"[xgb3-mstr] PR-AUC gap train-test = {train_test_gap:+.4f}  "
          f"(big positive = overfit train)")
    print(f"[xgb3-mstr] PR-AUC gap CV-test    = {cv_test_gap:+.4f}  "
    ...
    print()
    print(f"[xgb3-mstr] classification metrics @ fixed thr={THRESHOLD}:")
    ...
    print(f"[xgb3-mstr] curves saved to {curves_path}")
    print()

    print("[xgb3-mstr] TEST predictions, one row per wallet, sorted by prob desc.")
    print("[xgb3-mstr]  rank  prob    label  cum_tp  cum_fp  prec   recall  market  wallet")
    ...
    for rank, k in enumerate(order, start=1):
        ...
        print(f"[xgb3-mstr]  {rank:>4}    {proba_test[k]:.4f}    {lab}     "
              f"{cum_tp:>4}    {cum_fp:>4}  {prec:.3f}   {rec:.3f}  "
              f"{mkt_str:<30}  {wallet_str}")
    print()

    print(f"[xgb3-mstr] TEST confusion matrix @ thr={THRESHOLD}")
    ...
    print("[xgb3-mstr] feature importances (gain):")
    ...
    print()
    print(f"[xgb3-mstr] saved {model_path}")
    print(f"[xgb3-mstr] saved {calibrator_path}")
    print(f"[xgb3-mstr] saved {meta_path}")
    print(f"[xgb3-mstr] copied -> {latest_model}")
    print(f"[xgb3-mstr] copied -> {latest_calibrator}")
    print(f"[xgb3-mstr] copied -> {latest_meta}")
    print()
    print("[xgb3-mstr] comparison vs other models on each model's own test set "
