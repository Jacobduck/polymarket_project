"""Top-20 XGBoost variant with NO probability calibration.

Identical pipeline to xgboost3.py -- same Optuna ranges, StratifiedGroupKFold,
pinned iran-mar-1 test set, top 20 features -- but the OOF Platt calibrator
is removed. Raw XGBoost probabilities are used directly for both metrics and
thresholded classification.

Why drop the calibrator?
  xgb3 produced near-perfect ranking (PR-AUC=0.9969) but F1=0.0 at thr=0.5,
  because the Platt sigmoid (coef=4.62, intercept=-3.51) mapped raw prob 0.5
  to calibrated ~0.23, so no test row crossed thr=0.5. PR-AUC / ROC-AUC are
  invariant under Platt (it's a monotonic transform), so removing it costs
  nothing on the ranking metrics, and lets us see what raw probs do at thr=0.5.

Outputs:
  cache/models/xgb_insider_20feat_v2_nocal_<ts>.json
  cache/models/xgb_insider_20feat_v2_nocal_<ts>.meta.json
  cache/models/xgb_insider_20feat_v2_nocal_latest.json (copies)
  cache/models/xgb_insider_20feat_v2_nocal_latest.meta.json
  xgboost4_<ts>.log
  xgboost4_<ts>_curves.png
"""
...
TOP_K = 20
MODEL_TAG = f"xgb_insider_{TOP_K}feat_v2_nocal"
...
def get_top_features() -> list[str]:
    ...
    top = feats[:TOP_K]
    print(f"[xgb4] using top-{TOP_K} features from {FEATURE_SOURCE_META.name}:")
...
def split_markets_stratified_random(
    ...
    if PIN_TEST_MARKETS:
        ...
        print(f"[xgb4] PIN_TEST_MARKETS active -> "
...
def eval_split(
    ...
    out["f1"] = cls["f1"]
    print(f"[xgb4] {label:5s}  n={out['n']:4d} pos={out['n_positives']:3d}  "
...
def main() -> None:
    ...
    try:
        ...
        log_file.close()
        print(f"[xgb4] log written to {log_path}")


def _main(ts: str, log_path: Path) -> None:
    print(f"[xgb4] run timestamp = {ts}")
    print(f"[xgb4] logging to {log_path}")
    print(f"[xgb4] NO CALIBRATION: raw XGBoost probabilities used directly")
    print(f"[xgb4] loading {TRAIN_PARQUET}")
    df = pd.read_parquet(TRAIN_PARQUET)
    print(f"[xgb4] full shape={df.shape}  "
    ...
    print(f"[xgb4] row distribution (total n={n_total}, pos={pos_total}, "
    ...
    print(f"[xgb4] starting Optuna tuning: {N_OPTUNA_TRIALS} trials, "
    ...
    def _log_cb(study_: optuna.Study, trial: optuna.trial.FrozenTrial) -> None:
        ...
        print(f"[xgb4]   trial {trial.number:>3}  state={state:<8}  "
    ...
    print(f"[xgb4] tuning done in {tuning_secs:.1f}s  "
          f"({n_completed} completed, {n_pruned} pruned)")
    print(f"[xgb4] best CV PR-AUC = {best_cv_pr_auc:.4f}")
    print("[xgb4] best params:")
    ...
    print()
    print(f"[xgb4] re-running CV with best params to collect per-fold scores")
    splitter = StratifiedGroupKFold(
        n_splits=N_CV_FOLDS, shuffle=True, random_state=RANDOM_STATE,
    )
    ...
    print(f"[xgb4] CV PR-AUC = {cv_pr_auc_mean:.4f} ± {cv_pr_auc_std:.4f}")
    print()

    print(f"[xgb4] fitting final model on full train ({len(X_train)} rows)")
    final_model = build_xgb(best_params)
    t1 = time.time()
    final_model.fit(X_train, y_train, verbose=False)
    print(f"[xgb4] final fit in {time.time() - t1:.2f}s")
    print()

    # Raw XGBoost probabilities -- no calibration step.
    proba_train = final_model.predict_proba(X_train)[:, 1]
    proba_test = final_model.predict_proba(X_test)[:, 1]

    def _rng(a: np.ndarray) -> str:
        return f"[{a.min():.3f}, {a.max():.3f}]"
    print(f"[xgb4] raw prob range  train={_rng(proba_train)}  "
          f"test={_rng(proba_test)}")
    print()

    print("[xgb4] metrics per split (overfit check: train >> test => overfit):")
    ...
    print(f"[xgb4] PR-AUC gap train-test = {train_test_gap:+.4f}  "
          f"(big positive = overfit train)")
    print(f"[xgb4] PR-AUC gap CV-test    = {cv_test_gap:+.4f}  "
    ...
    print()
    print(f"[xgb4] classification metrics @ fixed thr={THRESHOLD}:")
    ...
    print(f"[xgb4] curves saved to {curves_path}")
    print()

    print("[xgb4] TEST predictions, one row per wallet, sorted by prob desc.")
    print("[xgb4]  rank  prob    label  cum_tp  cum_fp  prec   recall  market  wallet")
    ...
    for rank, k in enumerate(order, start=1):
        ...
        print(f"[xgb4]  {rank:>4}    {proba_test[k]:.4f}    {lab}     "
              f"{cum_tp:>4}    {cum_fp:>4}  {prec:.3f}   {rec:.3f}  "
              f"{mkt_str:<30}  {wallet_str}")
    print()

    print(f"[xgb4] TEST confusion matrix @ thr={THRESHOLD}")
    ...
    print("[xgb4] feature importances (gain):")
    ...
    meta = {
        ...
        "training_design": "stratified random train/test + StratifiedGroupKFold CV + Optuna (NO CALIBRATION)",
        "group_col": GROUP_COL,
        "split_strategy": (
    ...
    print()
    print(f"[xgb4] saved {model_path}")
    print(f"[xgb4] saved {meta_path}")
    print(f"[xgb4] copied -> {latest_model}")
    print(f"[xgb4] copied -> {latest_meta}")
    print()
    print("[xgb4] comparison vs other models on each model's own test set "
    ...
    comparisons = [
        ...
        ("xgb 10-feat v2 (xgboost2)",          MODEL_DIR / "xgb_insider_10feat_v2_latest.meta.json"),
        ("xgb 20-feat v2 (xgboost3, w/ cal)",  MODEL_DIR / "xgb_insider_20feat_v2_latest.meta.json"),
    ]
    for tag, path in comparisons:
        try:
            ...
            pass
    print(f"  {f'xgb {TOP_K}-feat v2 nocal (this run)':<37}  "
