"""XGBoost on the top-10 METADATA features only (no behavioral features).
...
scatter plots, sorted-prediction log, NaN ingested natively -- but the
feature set is ONLY the top-10 of the 17 metadata features (ranked by
xgb5's gain importance), with no behavioral features at all. This isolates
how much signal the metadata block carries on its own, versus the
behavioral model (xgb 20-feat v2) and the combined model (xgb5).

Outputs:
  cache/models/xgb_insider_meta10_v6_<ts>.json             (XGBoost native fmt)
  cache/models/xgb_insider_meta10_v6_<ts>.calibrator.joblib
  cache/models/xgb_insider_meta10_v6_<ts>.meta.json
  cache/models/xgb_insider_meta10_v6_latest.json           (copies)
  cache/models/xgb_insider_meta10_v6_latest.calibrator.joblib
  cache/models/xgb_insider_meta10_v6_latest.meta.json
  xgboost6_<ts>.log                                        (full stdout+stderr)
  xgboost6_<ts>_curves.png
"""
...
# feature set; no behavioral features are used.
...
def get_feature_columns() -> list[str]:
    """The top-10 metadata features only (no behavioral features).

    Validates the hardcoded list against METADATA_FEATURE_COLUMNS so a
    rename in the metadata module surfaces here instead of silently
    dropping a feature.
    """
    unknown = [c for c in TOP_METADATA_FEATURES if c not in METADATA_FEATURE_COLUMNS]
    if unknown:
        raise SystemExit(
            f"TOP_METADATA_FEATURES not in METADATA_FEATURE_COLUMNS: {unknown}"
        )
    feats = list(TOP_METADATA_FEATURES)
    print(f"[xgb6] using {len(feats)} metadata-only features "
          f"(top 10 of 17, ranked by xgb5 gain):")
...
def split_markets_stratified_random(
    ...
    if PIN_TEST_MARKETS:
        ...
        print(f"[xgb6] PIN_TEST_MARKETS active -> "
...
def eval_split(
    ...
    out["f1"] = cls["f1"]
    print(f"[xgb6] {label:5s}  n={out['n']:4d} pos={out['n_positives']:3d}  "
...
def main() -> None:
    ...
    try:
        ...
        log_file.close()
        print(f"[xgb6] log written to {log_path}")


def _main(ts: str, log_path: Path) -> None:
    print(f"[xgb6] run timestamp = {ts}")
    print(f"[xgb6] logging to {log_path}")
    print(f"[xgb6] loading {TRAIN_PARQUET}")
    if not TRAIN_PARQUET.exists():
        raise SystemExit(
            f"missing {TRAIN_PARQUET}; run build_dataset_with_metadata.py first"
        )
    df = pd.read_parquet(TRAIN_PARQUET)
    print(f"[xgb6] full shape={df.shape}  "
    ...
    print(f"[xgb6] markets total: {len(stats)}  "
          f"(train={len(train_mks)}, test={len(test_mks)})")
    print("[xgb6] market assignment (sorted by row count desc):")
    ...
    print(f"[xgb6] row distribution (total n={n_total}, pos={pos_total}, "
    ...
    print(f"[xgb6] starting Optuna tuning: {N_OPTUNA_TRIALS} trials, "
    ...
    def _log_cb(study_: optuna.Study, trial: optuna.trial.FrozenTrial) -> None:
        ...
        print(f"[xgb6]   trial {trial.number:>3}  state={state:<8}  "
    ...
    print(f"[xgb6] tuning done in {tuning_secs:.1f}s  "
          f"({n_completed} completed, {n_pruned} pruned)")
    print(f"[xgb6] best CV PR-AUC = {best_cv_pr_auc:.4f}")
    print("[xgb6] best params:")
    ...
    print()
    print(f"[xgb6] re-running CV with best params to collect OOF preds + per-fold scores")
    splitter = StratifiedGroupKFold(
        n_splits=N_CV_FOLDS, shuffle=True, random_state=RANDOM_STATE,
    )
    ...
    print(f"[xgb6] CV PR-AUC = {cv_pr_auc_mean:.4f} ± {cv_pr_auc_std:.4f}")
    ...
    print(f"[xgb6] OOF Platt calibrator: "
          f"sigmoid({cal_coef:.3f} * raw_prob {cal_intercept:+.3f})")
    print()

    print(f"[xgb6] fitting final model on full train ({len(X_train)} rows)")
    final_model = build_xgb(best_params)
    t1 = time.time()
    final_model.fit(X_train, y_train, verbose=False)
    print(f"[xgb6] final fit in {time.time() - t1:.2f}s")
    print()
    ...
    print(f"[xgb6] raw  prob range  train={_rng(proba_train_raw)}  "
          f"test={_rng(proba_test_raw)}")
    print(f"[xgb6] cal. prob range  train={_rng(proba_train)}  "
    ...
    print(f"[xgb6] sanity: TEST PR-AUC raw={raw_test_pr:.4f}  "
          f"(should equal calibrated PR-AUC; AUC is invariant to Platt)")
    print()

    print("[xgb6] metrics per split (overfit check: train >> test => overfit):")
    ...
    print(f"[xgb6] PR-AUC gap train-test = {train_test_gap:+.4f}  "
          f"(big positive = overfit train)")
    print(f"[xgb6] PR-AUC gap CV-test    = {cv_test_gap:+.4f}  "
    ...
    print()
    print(f"[xgb6] classification metrics @ fixed thr={THRESHOLD}:")
    ...
    print(f"[xgb6] curves saved to {curves_path}")
    print()

    print("[xgb6] TEST predictions, one row per wallet, sorted by prob desc.")
    print("[xgb6]  rank  prob    label  cum_tp  cum_fp  prec   recall  market  wallet")
    ...
    for rank, k in enumerate(order, start=1):
        ...
        print(f"[xgb6]  {rank:>4}    {proba_test[k]:.4f}    {lab}     "
              f"{cum_tp:>4}    {cum_fp:>4}  {prec:.3f}   {rec:.3f}  "
              f"{mkt_str:<30}  {wallet_str}")
    print()

    print(f"[xgb6] TEST confusion matrix @ thr={THRESHOLD}")
    ...
    print("[xgb6] feature importances (gain):")
    ...
    meta = {
        ...
        "feature_selection": (
            "top 10 of 17 metadata features, ranked by xgb5 gain "
            "(no behavioral features)"
        ),
        "n_behavioral_features": 0,
        "n_metadata_features": len(feature_cols),
        "metadata_features": list(feature_cols),
        "threshold": THRESHOLD,
    ...
    print()
    print(f"[xgb6] saved {model_path}")
    print(f"[xgb6] saved {calibrator_path}")
    print(f"[xgb6] saved {meta_path}")
    print(f"[xgb6] copied -> {latest_model}")
    print(f"[xgb6] copied -> {latest_calibrator}")
    print(f"[xgb6] copied -> {latest_meta}")
    print()
    print("[xgb6] comparison vs other models on each model's own test set "
    ...
    for tag, path in comparisons:
        try:
            ...
            print(f"  {tag:<40}  ROC-AUC={m.get('roc_auc', 0):.4f}  "
                  f"PR-AUC={m.get('pr_auc', 0):.4f}")
        except FileNotFoundError:
            pass
    print(f"  {'xgb metadata-only v6 (this run)':<40}  "
